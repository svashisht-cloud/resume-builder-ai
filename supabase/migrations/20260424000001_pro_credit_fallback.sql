-- Migration: Pro credit fallback
-- When a Pro user hits the 100 resume/month limit, the system falls back to
-- spending a credit instead of hard-blocking (P0004 removed entirely).
-- restore_credit is updated to handle the fallback path correctly.

-- ── start_or_regen_resume (replacement) ──────────────────────────────────────
-- Pro branch: plan_monthly_usage < 100 → free (plan path).
--             plan_monthly_usage >= 100 → spend_credit() fallback (P0001 if none).
-- P0004 is no longer raised anywhere.
...
BEGIN
  ...
  IF v_is_pro THEN
    ...
    IF v_existing_id IS NOT NULL THEN
      IF p_force_fresh THEN
        -- Try plan increment first; fall back to credit at limit
        ...
        IF v_new_usage IS NULL THEN
          -- At monthly limit: spend a credit (P0001 if none available)
      ...
      -- Regen: always free for Pro (no monthly_usage, no credit)
  ...
  ELSE
    -- ── CREDIT PATH (P0003 removed — free credits allow regen) ───────────────
...
-- ── restore_credit (replacement) ─────────────────────────────────────────────
-- Pro path: if a fallback credit was spent on this resume (within 5 min),
--           restore it. Otherwise decrement plan_monthly_usage (normal plan path).
-- Credit path: unchanged.
...
BEGIN
  ...
  IF v_is_pro THEN
    ...
    END IF;
  ELSE
    -- Credit path: un-spend the credit within the window
    UPDATE public.credits
       SET spent_at = null, spent_on_resume_id = null
     WHERE spent_on_resume_id = p_resume_id
       AND user_id = v_caller
